...
HttpTransport::HttpTransport(const std::string& _host, const std::string& _port,
                             const std::string& _target, const std::string& _protocol) :
    host{_host}, port{_port}, target{_target}, protocol{_protocol}
{
}
      ...
      [=]()
      {
        ...
        auto build_request = [&]()
        {
          http::request<http::string_body> req{
              (reqType == RequestTypes::POST) ? http::verb::post : http::verb::get, target, 11};
...
std::string HttpTransport::getProtocol() const
{
  return protocol;
}
